...
$(function () {
	...
	var res = [{ id: 'bg', src: './css/bg.jpg' }, { id: 'logo', src: './css/logo.png' }, { id: 'page01-top', src: './css/page01-top.png' }];
	var hengda20list = [];
	(function () {
		var i = 37;
		var _i = '';
		while (i > 0) {
			...
			hengda20list.push('./css/20/' + _i + '.png');
			i--;
		}
	})();
	loader.startLoad(res).handlerLoadComplete(function () { //使用startLoad预加载文件，加载完成后执行handlerLoadComplete里面的方法
		...
		hengda20list = hengda20list.map(function (item, i) {  //map方法转化20图片路径为数组
			return '<i style="background-image:url(' + item + ')"></i>';
		});
		console.log(hengda20list);

		$('.hengda20 .inner').html(hengda20list.join(''));  //把20图片的Html代码插入到页面上
	});

	var ua = navigator.userAgent;
	window.addEventListener("devicemotion", function (e) {			//重力感应
